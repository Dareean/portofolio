...
export default function AboutBento({ aboutData }: AboutBentoProps) {
  ...
  useEffect(() => {
    const timeZone = aboutData?.timeZone || "Asia/Makassar";
    const updateTime = () => {
      const now = new Date();
  ...
  const portrait = aboutData?.portraitImage || "/assets/foto_closeup.jpg";
  const name = aboutData?.fullName || "Dareean Ahmad Raffi";
  const roleTag = aboutData?.roleTag || "Full-Stack & UI/UX";
  const location = aboutData?.locationText || "Palu, Central Sulawesi · 0°53' S";
  const philosophyHeading = aboutData?.philosophyHeading || "From pixel to people — turning complex engineering into human-centered software.";
  const philosophyBio = aboutData?.philosophyBio || "I see code not merely as syntax, but as a bridge between ideas and reality. Based in Central Sulawesi, I combine formal computer science fundamentals with hands-on product craft.";
  const resumeUrl = aboutData?.resumeUrl || "/assets/CV-Rafi(English).pdf";
  const statusTag = aboutData?.statusTag || "Remote Ready";
  const communityRoles = aboutData?.communityRoles || [
          ...
          >
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-micro-uppercase text-steel font-semibold tracking-wider font-mono">
                  Philosophy &amp; Ethos
                </span>
              </div>
          ...
          >
            <div>
              ...
              <div className="p-4 rounded-lg bg-canvas border border-hairline">
                <div className="flex items-center gap-2 text-micro-uppercase text-steel font-semibold mb-1 font-mono">
          ...
          >
            <div>
              ...
              <div className="space-y-2 mt-4">
                {communityRoles.map((cr, idx) => (
                  <div key={idx} className="flex items-center justify-between text-body-sm p-2.5 rounded bg-canvas border border-hairline">
                    <span className="text-charcoal font-medium">{cr.org}</span>
                    <span className="text-caption text-steel font-mono">{cr.role}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                {techStack.map((tech) => (
                  ...
                  >
                    {DEFAULT_TECH_LOGOS[tech.name] || <Code2 className="w-3 h-3 text-steel" />}
